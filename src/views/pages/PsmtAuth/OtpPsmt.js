...
import { VerifyPsmtOTP } from '../../../redux/actions/authAction';
import { useDispatch, useSelector } from 'react-redux';
import OtpInput from 'react-otp-input';
import "../../components/InputText/InputStyle.scss"
import "./Auth.scss"

function OtpPsmt(props) {
	...
	const handSubmit = (e) => {
		setSubmitClick(true)
		e.preventDefault();
		dispatch(VerifyPsmtOTP(getOtp))
	}

	if(submitClick == true) {
		console.log("clicked response ",OTPApiResponse);
		if((OTPApiResponse && OTPApiResponse.status == 200) && ( OTPApiResponse && OTPApiResponse.message == "Otp Verified Success.")){
							...
							<div className="auth-form otp-form">
								<Form autoComplete="off" onSubmit={handSubmit}>
									...
									<div className="bottom-info text-start">
										<p> Not received your code? <Link to="/" className="secondary-text">Resend </Link></p>
									</div>
